...
interface AttendanceContextType {
  ...
  checkBiometricCapabilities: () => Promise<{ hasHardware: boolean; isEnrolled: boolean; supportedTypes: string[] }>
  loading: boolean
}
...
export function AttendanceProvider({ children }: { children: ReactNode }) {
  ...
  const checkBiometricCapabilities = async () => {
    ...
    return {
      ...
      supportedTypes: capabilities.supportedTypes.map((type) => type.toString()),
    }
  }
    ...
    try {
      ...
      if (!result.success) {
        if (result.error === "UserCancel") {
          ...
          throw new Error("Biometric authentication is required for attendance")
        } else {
      ...
      await addDoc(collection(db, "attendance"), {
        ...
        biometricType: BiometricService.getBiometricTypeString(capabilities.supportedTypes),
      })
    } catch (error) {
      throw error
    } finally {
  ...
  const getTodayAttendance = async (teacherId: string): Promise<AttendanceRecord | null> => {
    try {
      const today = new Date().toISOString().split("T")[0]
      const q = query(collection(db, "attendance"), where("teacherId", "==", teacherId), where("date", "==", today))
